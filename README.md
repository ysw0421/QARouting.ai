...
- **한계**
    - 현재는 룰/키워드 기반 컴플라이언스 탐지이나, GPT-4.1 nano 등 LLM 기반 탐지로 확장 가능
    - 약관 변경 추출은 현재 전체 텍스트 반환이나, LLM 기반 diff/변경점 추출로 고도화 가능
    - API/웹 연동, 실시간 알림 등은 미구현(향후 LLM 기반 워크플로우 자동화 및 외부 시스템 연동 가능)
- **확장 방향**
